...
async def stream_audio(kokoro, text, voice, speed, lang, debug=False):
    # Initialize the playback backend only for --stream. Headless servers can
    # still generate WAV/MP3 files without a running PulseAudio server.
    try:
        import sounddevice as sd
    except (ImportError, OSError) as exc:
        raise RuntimeError(
            "Audio streaming requires a working PortAudio/PulseAudio device; "
            "save to a WAV or MP3 file on headless servers."
        ) from exc
